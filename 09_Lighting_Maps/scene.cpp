...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		// draw light
		glm::mat4 lightModel = glm::translate(model, Lights::defaultLight.position);
		lightModel = glm::scale(lightModel, glm::vec3(0.2f));
		lightShader.use();
		lightShader.setMat4("model", lightModel);
		lightShader.setMat4("view", cam.getViewMatrix());
		lightShader.setMat4("projection", projection);
		lightShader.setLight("light", Lights::defaultLight);
		...
		lightShader.setVec3("light.ambient", lightColor * glm::vec3(0.2f));
		lightShader.setVec3("light.diffuse", lightColor * glm::vec3(0.5f));
#endif
		glBindVertexArray(lightVAO);
		...
		objectShader.setLight("light", Lights::defaultLight);
#if LIGHT_COLOR_CAHNGE
